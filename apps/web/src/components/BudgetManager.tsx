...
export default function BudgetManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Failed to create budget';
      setError(msg);
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="card card-sticky">
        ...
        {showForm ? (
          <form onSubmit={handleSubmit} className="budget-form">
            ...
            <label className="form-label" style={{ marginTop: 12 }}>Monthly Limit (USD)</label>
            <input
      ...
      <div>
        {budgets.length === 0 ? (
          <div className="card" style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-3)' }}>
            ...
            <div style={{ fontSize: 14 }}>No budgets yet.</div>
            <div style={{ fontSize: 12, marginTop: 4 }}>Add a budget to start tracking your spending limits.</div>
          </div>
        ) : (
                ...
                <div key={b.id} className={`budget-card${over ? ' budget-card--over' : warning ? ' budget-card--warning' : ''}`}>
                  <div className="budget-card-header">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      {over && <span className="budget-badge budget-badge--over">Over budget</span>}
                      {warning && <span className="budget-badge budget-badge--warning">Near limit</span>}
                    </div>
                    <button
